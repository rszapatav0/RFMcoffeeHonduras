...
#sales_process_df         <- read_excel(excel_path, sheet = "sales_process_repeat") %>%
#  rename(typeCoffeeSoldToBuyer = form1)
...
lands_dict       <- dictionary %>% filter(el_loop == "lands")
aggregated_lands <- aggregate_data(lands_df, lands_dict, "_submission__uuid", "_trr")
colnames(aggregated_lands)

#repeatvar
repeatvar_df         <- read_excel(excel_path, sheet = "repeatvar")
repeatvar_dict       <- dictionary %>% filter(el_loop == "repeatvar")
aggregated_repeatvar <- aggregate_data(repeatvar_df, repeatvar_dict, "_submission__uuid", "_var")
colnames(aggregated_repeatvar)

#pnd_repeat
pnd_repeat_df         <- read_excel(excel_path, sheet = "pnd_repeat")
pnd_repeat_dict       <- dictionary %>% filter(el_loop == "pnd_repeat")
aggregated_pnd_repeat <- aggregate_data(pnd_repeat_df, pnd_repeat_dict, "_submission__uuid", "_dop")
colnames(aggregated_pnd_repeat)

#tech_repeat
tech_repeat_df         <- read_excel(excel_path, sheet = "tech_repeat")
tech_repeat_dict       <- dictionary %>% filter(el_loop == "tech_repeat")
aggregated_tech_repeat <- aggregate_data(tech_repeat_df, tech_repeat_dict, "_submission__uuid", "_tec")
